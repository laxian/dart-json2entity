class JsonF6 {
  ...
  Map<String, dynamic> toJson() =>
      {'message': message, 'data': data?.map((it) => it.toJson())?.toList()};
}
  ...
  DataEntity.fromJson(Map<String, dynamic> json)
      : title = json['title'],
        ...
        image_list = (json['image_list'] as List)
            ?.map((l) => Image_listEntity.fromJson(l))
            ?.toList();
  Map<String, dynamic> toJson() => {
        'title': title,
...
class Image_listEntity {
  ...
  Image_listEntity.fromJson(Map<String, dynamic> json) : url = json['url'];
